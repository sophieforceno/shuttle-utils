...
print_stats () {
	...
	for i in "${CL_REPO[@]}"; do
		session_stats=$(ls -lth "$i"/rdiff-backup-data/session_statistics* | awk '{ print $9 }' | head -n1)
	# INFO: Uncomment to receive session stats via email
		# sudo cat "$session_stats" | mail -s "Latest backup stats for $i" root@aurorabox	
	done
			if [[ "$output" = "push" ]]; then
			cat "$session_stats" | shuttle -p -n "$device" "Latest backup stats for $backup" 
...
case "$2" in
	-l|--log)
		print_log
		for i in "${CL_REPO[@]}"; do
			cat "$i"/rdiff-backup-data/backup.log
	...
	-e|--errors)
		print_log
		for i in "${CL_REPO[@]}"; do
			cat "$i"/rdiff-backup-data/backup.log | grep -A4 -E 'exceptions.IOError|exceptions.OSError' 
			echo ""
		done
		;;
	*)
...
case "$3" in
	...
	-e|--errors)
		print_log
		for i in "${CL_REPO[@]}"; do
			cat "$i"/rdiff-backup-data/backup.log | grep -A4 exceptions.IOError
		done
	;;
esac	
