...
for i in "${HOSTS[@]}"; do
echo -e "smartmon_health: Reading SMART data: $i"
	...
	if [[ "$(which smartctl)" = \/* ]]; then
		...
		for j in "${TO_SCAN[@]}"; do
			...
			if [[ "$realloc_delta" > 0 || "$pending_delta" > 0 || "$offline_delta" > 0 ]]; then
				...
				printf "%s\n" "Reallocated: $realloc_delta, Pending: $pending_delta, Offline: $offline_delta" | "$LC_shuttle_path"/shuttle -p -n "$LC_device" "$LC_i: $j: SMART values have changed!"
			fi
		done
